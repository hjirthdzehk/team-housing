...
object Billing  extends SQLSyntaxSupport[Billing]{
  def apply()(rs: WrappedResultSet): Billing =
    ...
    def getPersonDebt(personId: Long)(implicit session: DBSession = autoSession) : Option[Billing] = {
      sql"""
          SELECT p.person_id as "personId",
                   ...
                   GROUP BY m1.meter_id, meter.title, p.person_id, p.name, p.surname, flat.flat_number
